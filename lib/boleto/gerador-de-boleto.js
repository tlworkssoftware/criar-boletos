	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				const logo = beneficiario.getLogo();

				let ajusteEndBenificiario = 27;

				if (logo && logo !== '') {
					ajusteEndBenificiario = 95;
					
					pdf.image(logo, args.ajusteX + 27, args.ajusteY + zeroLinha + args.tamanhoDaFonte + 1.5, {
						height: 30,
						width: 60
					});
				}
				...
				pdf.font('normal')
					...
					.text(beneficiario.getIdentificacao().toUpperCase(), args.ajusteX + ajusteEndBenificiario, args.ajusteY + zeroLinha + args.tamanhoDaFonte + 1.5, {
						lineBreak: false,
						width: 545,
						align: 'left'
					});
				...
				if (enderecoDoBeneficiario) {
					...
					if (enderecoDoBeneficiario.getPrimeiraLinha()) {
						...
						pdf.font('normal')
							.fontSize(args.tamanhoDaFonte)
							.text(enderecoDoBeneficiario.getPrimeiraLinha(), args.ajusteX + ajusteEndBenificiario, args.ajusteY + zeroUmLinha, {
								lineBreak: false,
								width: 545,
								align: 'left'
							});
					...
					if (enderecoDoBeneficiario.getSegundaLinha()) {
						pdf.font('normal')
							.fontSize(args.tamanhoDaFonte)
							.text(enderecoDoBeneficiario.getSegundaLinha(), args.ajusteX + ajusteEndBenificiario, args.ajusteY + zeroDoisLinha, {
								lineBreak: false,
								width: 545,
								align: 'left'
							});
				...
				const codigoDeBarras = boleto.getCodigoDeBarras();
				...
				pdf.font('negrito')
					.fontSize(args.tamanhoDaLinhaDigitavel)
					.text(boleto.getLinhaDigitavel(), args.ajusteX + margemDoSegundoBlocoLayout + 145, args.ajusteY + segundaLinha2, {
						lineBreak: false,
						width: 400,
						align: 'right'
					});
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(beneficiario.getUsoBanco(), args.ajusteX + tituloUsoDoBancoX, args.ajusteY + sextaLinha, {
						lineBreak: false,
						width: 71,
						align: 'center'
					});
